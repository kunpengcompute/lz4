...
- [2026.05.29]：发布LZ4 v1.9.4鲲鹏优化版，包含NEON向量化内存拷贝、分支预测、数据预取等优化。

## 项目简介

### 简介

LZ4是由Yann Collet开发的**极快速无损压缩算法**，属于LZ77家族，以压缩速度著称（单核可达500MB/s以上），同时提供合理的压缩率。其核心优势为压缩/解压速度极快、内存占用小、支持流式与块式两种压缩模式，且提供了跨C/C++/Java/Python等多语言的绑定。

### 核心模块

LZ4整体分为**压缩模块**和**解压缩模块**两大核心，其内部模块划分如下：

- **压缩模块**：提供块压缩（LZ4_compress_default/fast/HC）、流式压缩（LZ4_compress_continue/fastContinue）等基础能力；
- **解压缩模块**：提供块解压（LZ4_decompress_safe/fast）、部分解压（LZ4_decompress_safe_partial）等基础能力；
- **哈希模块**：实现4字节/5字节快速哈希计算，用于压缩时的匹配查找；
- **内存拷贝模块**：实现wildCopy8/wildCopy16/wildCopy32/wildCopy64等高效数据拷贝路径；
- **帧格式模块**：提供LZ4帧格式的压缩/解压接口（lz4frame），支持流式操作与校验。

### 对外核心接口函数

以下为LZ4（以C为例）对外的核心接口函数，覆盖压缩、解压缩、流式操作等核心能力。
...
基于鲲鹏优化的LZ4每个发布版本特性变更详细信息，请参见《[版本说明书](docs/zh/release_notes.md)》。

## 环境部署

基于鲲鹏优化的LZ4编译环境、依赖获取与安装步骤参见《[安装指南](docs/zh/installation_guide.md)》。
...
|资源名称|资源简介|
|--|--|
|[版本说明书](docs/zh/release_notes.md)|提供基于鲲鹏优化的LZ4每个发布版本的基础信息和特性更新说明。|
|[快速入门](docs/zh/quick_start.md)|提供基于鲲鹏优化的LZ4快速上手示例与编译运行说明。|
|[API参考](docs/zh/api_reference.md)|提供NEON向量化内存拷贝、哈希优化等相关宏与函数的接口说明与定义。|
|[安装指南](docs/zh/installation_guide.md)|提供基于鲲鹏优化代码LZ4的环境配置与编译安装的详细指导。|
